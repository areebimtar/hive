...
import { ProductRowPriceInventory } from '../ProductRowPriceInventory';
...
describe('ProductRowPriceInventory', () => {
  it('should render Thumbnail', () => {
    const product = fromJS({can_write_inventory: true, _status: {data: {}}});
    const wrapper = shallow(<ProductRowPriceInventory product={product}/>);
  ...
  it('should not render global error message', () => {
    const product = fromJS({can_write_inventory: true, _status: {data: {}}, _formattedPriceInventory: [{showValue: false}]});
    const wrapper = shallow(<ProductRowPriceInventory product={product} />);
    expect(wrapper.find('.global-status .error')).to.have.length(0);
  });

  it('should render global error message', () => {
    const product = fromJS({can_write_inventory: true, _status: {data: {status: 'error'}}, _inInlineEditing: true, _formattedPriceInventory: [{showValue: false}]});
    const wrapper = shallow(<ProductRowPriceInventory product={product} />);
