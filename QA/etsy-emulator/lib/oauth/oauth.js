...
exports.AUTH_URL = 'bbbbbbbbbbbbbb';

exports.requestToken = [
  passport.authenticate('consumer', { session: false }),
  server.requestToken(function(client, callbackURL, done) {
	  //XXX console.log('client=', client);
	  //XXX console.log('AUTH_URL=', exports.AUTH_URL);
    // var token = utils.uid(8) , secret = utils.uid(32)
    var token = 'TmpRequestToken1'
      , secret = 'TmpRequestSecret1'
	  , params = {
          login_url: exports.AUTH_URL + '?oauth_consumer_key=' + client.consumerKey + '&oauth_token=' + token  + '&service=v2_prod',
    ...
    db.requestTokens.save(token, secret, client.id, callbackURL, function(err) {
      if (err) { return done(err); }
      return done(null, token, secret, params);
    });
  }),
  server.errorHandler()
]
...
exports.accessToken = [
  ...
  server.accessToken(
    ...
    function(client, requestToken, info, done) {
		//XXX console.log('accessToken 2: client = ', client, ' requestToken = ', requestToken, ' info = ', info);
      if (!info.approved) { return done(null, false); }
      if (client.id !== info.clientID) { return done(null, false); }
      
      var token = 'MyAccesToken1' , secret = 'MyAccessTokenSecret1';
      //var token = utils.uid(16) , secret = utils.uid(64)
