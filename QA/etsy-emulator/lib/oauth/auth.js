...
passport.use('token', new TokenStrategy(
  ...
  function(consumerKey, done) {
    db.clients.findByConsumerKey(consumerKey, function(err, client) {
      ...
      if (err) { return done(err); }
      if (!client) { return done(null, false); }
	  //XXX console.log('findByConsumerKey result OK: err = ', err, ' client=', client);
      return done(null, client, client.consumerSecret);
    });
  },
  // verify callback
  ...
  function(accessToken, done) {
	  //XXX console.log('find access token accessToken = ', accessToken);
    db.accessTokens.find(accessToken, function(err, token) {
	  //XXX console.log('find access token err = ', err , ' token = ', token);
      if (err || (token == null)) { return done(err); }
      db.users.find(token.userID, function(err, user) {
        if (err) { return done(err); }
        if (!user) { return done(null, false); }
        // to keep this example simple, restricted scopes are not implemented
        var info = { scope: '*' }
	  //XXX console.log('\nfind access token result OK: user = ', user, '\ntoken.secret = ', token.secret, '\ninfo = ', info);
