...
const addOp = (product, op, value, noFormatting) => {
  ...
  let result = product;
  // convert comma sepparated tags in string into array
  ...
  if (!noFormatting) {
    ...
    result = result.set('_formattedTags', fromJS(_.uniq(existingTags.concat(newTags), (tag) => tag.name.toLowerCase()).splice(0, BULK_EDIT_VALIDATIONS.TAGS_MAX_LENGTH)));
  }
  // append new tags to products.tags array
  result = result.set('tags', fromJS(_.uniq(currentTags.concat(tagsArr), (tag) => tag.toLowerCase()).splice(0, BULK_EDIT_VALIDATIONS.TAGS_MAX_LENGTH)));
