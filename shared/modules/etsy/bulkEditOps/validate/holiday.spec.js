...
import module, { validate } from './holiday';

chai.use(sinonChai);

describe('Holiday - validate', () => {
  describe('validate', ()=> {
    let getAttribute;
    ...
    it('should not set error if there is no holiday attribute', () => {
      const availableOptions = [{ id: 1 }, { id: 2 }, { id: 3 }];
      getAttribute.returns(fromJS({ availableOptions }));
    ...
    it('should not set error if there is valid holiday attribute', () => {
      const availableOptions = [{ id: 1 }, { id: 2 }, { id: 3 }];
      getAttribute.returns(fromJS({ availableOptions }));
      const attributes = [{ propertyId: '46803063659', valueIds: ['1']}];
      const product = fromJS({ can_write_inventory: true, attributes });
      const error = validate(product);
    ...
    it('should set error if there is invalid holiday attribute', () => {
      const availableOptions = [{ id: 1 }, { id: 2 }, { id: 3 }];
      getAttribute.returns(fromJS({ availableOptions }));
      const attributes = [{ propertyId: '46803063659', valueIds: ['4']}];
      const product = fromJS({ can_write_inventory: true, attributes });
      const error = validate(product);

      expect(getAttribute).to.have.been.called;
      expect(error).to.eql('Holiday cannot be changed due to category');
    });

    it('should set error if there are no valid options', () => {
      ...
      expect(getAttribute).to.have.been.called;
      expect(error).to.eql('The category of this listing does not support holiday');
    });
  });
});
