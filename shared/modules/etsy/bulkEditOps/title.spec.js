...
describe('BulkEditOps - Title', () => {
  ...
  describe('validate', () => {
    ...
    it('should validate title if input does not start with alphanum character', () => {
      title.__Rewire__('BULK_EDIT_VALIDATIONS', {TITLE_MAX_LENGTH: 5});
      const status = title.validate(fromJS({title: ' sdf'})).toJS();

      expect(getFormattedInvalidTitleMessage).not.have.been.called;
      expect(status.valid).to.be.false;
      expect(status.data).to.be.string;
    });

    it('should validate title if input contains more than three words with all capital letters', () => {
      title.__Rewire__('BULK_EDIT_VALIDATIONS', {TITLE_MAX_LENGTH: 140});
      expect(title.validate(fromJS({title: 'sjkdfh skjdfh weioru sdfio Wasd Ids Asdi Idsd'})).toJS().valid).to.be.true;
      expect(title.validate(fromJS({title: 'Sjkdfh Skjdfh Weioru Sdfio Wasd Ids Asdi Idsd'})).toJS().valid).to.be.true;
      expect(title.validate(fromJS({title: 'sdh TEST FOO basjk BAR hjsdh Esd ghdg'})).toJS().valid).to.be.true;
      expect(title.validate(fromJS({title: 'sdh TEST FOO basjk BAR hjsdh Esd GHDG'})).toJS().valid).to.be.false;
    });

    it('should set error if input contains & % : more than once', () => {
      title.__Rewire__('BULK_EDIT_VALIDATIONS', {TITLE_MAX_LENGTH: 140});
      expect(title.validate(fromJS({title: 'ssdk & skdl'})).toJS().valid).to.be.true;
      expect(title.validate(fromJS({title: 'ssdk && skdl'})).toJS().valid).to.be.false;
      expect(title.validate(fromJS({title: 'ssdk & wer& skdl'})).toJS().valid).to.be.false;
      expect(title.validate(fromJS({title: 'ssdk : skdl'})).toJS().valid).to.be.true;
      expect(title.validate(fromJS({title: 'ssdk :: skdl'})).toJS().valid).to.be.false;
      expect(title.validate(fromJS({title: 'ssdk : wer: skdl'})).toJS().valid).to.be.false;
      expect(title.validate(fromJS({title: 'ssdk % skdl'})).toJS().valid).to.be.true;
